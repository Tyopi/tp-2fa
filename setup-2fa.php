...
if (empty($_SESSION['tfa_secret_temp'])) {
    s$_SESSION['tfa_secret_temp'] = $tfa->createSecret();
}
...
echo "<p>Compte : <strong>" . htmlspecialchars($email, ENT_QUOTES, 'UTF-8') .
"</strong></p>";
echo "<p>Clé secrète : <code>" . htmlspecialchars($secret, ENT_QUOTES, 'UTF-8') .
"</code></p>";
echo "<p>Longueur : " . strlen($secret) . " caractères</p>";
echo "<p>Valide (A-Z2-7) : " . (preg_match('/^[A-Z2-7]+$/', $secret) ? ' Oui' : ' Non')✅ ❌
. "</p>";
echo "<h3> Scanne le QR code</h3>";📱
echo "<img src='qrcode.png?v=" . time() . "' alt='QR Code 2FA'><br><br>";
echo "<p>Si le QR ne marche pas, saisis la clé manuellement : <code>" .
htmlspecialchars($secret, ENT_QUOTES, 'UTF-8') . "</code></p>";
echo "<p>URL OTP (debug) : <code>" . htmlspecialchars($otpauthUrl, ENT_QUOTES, 'UTF-8') .
"</code></p>";
...
echo "<input type='text' name='code' id='code' inputmode='numeric' pattern='[0-9]{6}'
